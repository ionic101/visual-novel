...
label company_party:
    ...
    boss "Ты очень мне помог, и я, как обещал, тебя за это награжу. Одной премией ты не отделаешься, теперь ты будешь главным разработчиком этого проекта. Нужно совершенствовать систему и поддерживать её постоянную работу."
